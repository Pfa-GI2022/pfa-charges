...
export class ListeSousModulesComponent implements OnInit {
  ...
  currentUrl = '';
  hide = false;
  constructor(private route: ActivatedRoute, private moduleService: ModuleService, private matiereService: MatiereService,private calculeCharge:CalculeChargeService,private Location:Location,private router:Router) {
    ...
    router.events.forEach((event) => {
      if(event instanceof NavigationEnd) {
        this.currentUrl = this.Location.path().split('/')[this.Location.path().split('/').length-1]
